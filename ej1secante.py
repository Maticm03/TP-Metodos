...
def find_intersection_points(x1_interp_med, x2_interp_med):
    """
    Find all intersection points between the measured curve and the analytical lines using the secant method.
    ...
    # Secant method to find intersection points with the magenta line
    for i in range(1, len(x2_interp_med)):
        if np.sign(x2_interp_med[i] - x2_mag[i]) != np.sign(x2_interp_med[i - 1] - x2_mag[i - 1]):
            # Apply the secant method to find the root
            x1_point1, x2_point1 = x1_interp_med[i - 1], x2_interp_med[i - 1]
            x1_point2, x2_point2 = x1_interp_med[i], x2_interp_med[i]
            x1_intersection = x1_point1 - ((x1_point2 - x1_point1) / (x2_point2 - x2_point1)) * (x2_point1 - x2_mag[i - 1])
            x2_intersection = x2_mag[i - 1]
            
            if 0 <= x1_intersection <= 10:
                intersection_points.append((x1_intersection, x2_intersection))

    # Secant method to find intersection points with the green line (x1=10)
    for i in range(1, len(x1_interp_med)):
        if np.sign(x1_interp_med[i] - 10) != np.sign(x1_interp_med[i - 1] - 10):
            # Apply the secant method to find the root
            x1_point1, x2_point1 = x1_interp_med[i - 1], x2_interp_med[i - 1]
            x1_point2, x2_point2 = x1_interp_med[i], x2_interp_med[i]
            t = (10 - x1_point1) / (x1_point2 - x1_point1)
...
def create_continuous_curve_from_csv(csv_groundtruth, csv_mediciones):
    try:
        ...
        plt.plot(x1_interp_gt, x2_interp_gt, 'b-', label='Ground Truth', linewidth=2)
